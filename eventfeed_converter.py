...
def groups_converter(token_st):
    ...
    with open("./config/groups.csv", "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if row[2] == "1":
                groups_list.append({"type": "groups", "id": row[0], "data": { "id": row[0], "name": row[1], "hidden": True}, "token": f"cdA{token_st}"})
            else:
                groups_list.append({"type": "groups", "id": row[0], "data": { "id": row[0], "name": row[1]}, "token_st": f"cdA{token_st}"})
            token_st += 1
...
def state_generater(token_st):
    ...
    with open('./config/state_config.yaml', 'r', encoding='utf8') as file:
        ...
        state_list.append({"type": "state", "data": {"started": state_info['started']}, "token": get_token_str(token_st)})
        state_list.append({"type": "state", "data": {"started": state_info['started'], "frozen": state_info['frozen']}, "token": get_token_str(token_st + 1)})
        state_list.append({"type": "state", "data": {"started": state_info['started'], "frozen": state_info['frozen'], "ended": state_info['ended']}, "token": get_token_str(token_st + 2)})
        token_st += 3
